...
imkreg0Z1A <- function(y,exvar.beta=NA,exvar.nu=NA,exvar.rho=NA,tau=0.5,print=T,check=F,link="logit")
{
  n <- length(y) 
  ...
  dw<-function(res){
    ...
    Q1 <- chol2inv(qr.R(qr(cbind(exvar.beta,exvar.nu,exvar.rho))))
    if (n<100) {
      A <- diag(c(1, rep(2, n - 2), 1))
      A[abs(row(A) - col(A)) == 1] <- -1
      MA <- diag(rep(1, n)) - cbind(exvar.beta,exvar.nu,exvar.rho) %*% Q1 %*% t(cbind(exvar.beta,exvar.nu,exvar.rho))
      MA <- MA %*% A
      ev <- eigen(MA)$values[1:(n - (c+k+m))]
      if (any(Im(ev) > 1e-10)) 
      ...
      if (n < max(5, (c+k+m))) {
        ...
        AX <- matrix(as.vector(filter(cbind(exvar.beta,exvar.nu,exvar.rho), c(-1, 2, -1))), ncol = (k+c+m))
        AX[1, ] <- cbind(exvar.beta,exvar.nu,exvar.rho)[1, ] - cbind(exvar.beta,exvar.nu,exvar.rho)[2, ]
        AX[n, ] <- cbind(exvar.beta,exvar.nu,exvar.rho)[n, ] - cbind(exvar.beta,exvar.nu,exvar.rho)[(n - 1), ]
        XAXQ <- t(cbind(exvar.beta,exvar.nu,exvar.rho)) %*% AX %*% Q1
        P <- 2 * (n - 1) - sum(diag(XAXQ))
        Q <- 2 * (3 * n - 4) - 2 * sum(diag(crossprod(AX) %*% Q1)) + sum(diag(XAXQ %*% XAXQ))
        dmean <- P/(n - (c+k+m))
        dvar <- 2/((n - (c+k+m)) * (n - (c+k+m) + 2)) * (Q - P * dmean)
