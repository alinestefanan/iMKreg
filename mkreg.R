...
mkreg <- function(y,exvar=NA,tau=0.5,resid=1,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    ...
    for (i in 1:length(y)){
      ...
      l[i]=log(alpha)+alpha-alpha/y[i]+(log(1-tau)/den.cr[i] -1)*critical.ly[i]-2*log(y[i])+log(log(1-tau)/den.cr[i])
    }
     #print("l");print(sum(l))
  ...
  score <- function(z) 
  {
    ...
    mT <- diag(mu.eta(eta[1:n]))
    
    deta.dbeta0 <- matrix(rep(NA,n),ncol=1)#intercept
    for(i in 1:n)
    {
      deta.dbeta0[i,1] <- X[i,1] 
    }
    ...
    return(c(Ubeta0,Ubeta,Ualpha))
  }#end score
  
  # initial values for estimation
  x <- cbind(X[1:n,])#intercepto, covariáveis
 ...
 # reg <- c(mqo,0)
  ...
  obs.inf<-function(y,muhat)
  {
    ...
    mT <- diag(mu.eta(etahat[1:n]))    
    num.mT2=dif2link(muhat)
    mT2=diag(-num.mT2*(mu.eta(etahat[1:n])^3))
    ...
    K <- rbind(
      cbind(KB0B0,KB0B,KB0alpha),
      cbind(KBB0,KBB,KBalpha),
      cbind(KalphaB0,KalphaB,Kalphaalpha)
    )
    return(K)
  }
  K<-obs.inf(y,muhat)
  ...
  first_col<-c("intercept", 1:k,"alpha estimator")
...
#print(z$coef.result)
 ...
 # print(z$residual)
  ...
  if(graph==T)
  {
    ...
    w1<-5
    h1<-4
    pdf("cor_y_cov.pdf",width=5, height=4)
    {
      if (k<=3){
      par(mfrow=c(1,k))
      ...
      # }
      if(k==1){
      plot(as.vector(X[,2]),as.vector(y),main=" ",xlab=paste("Cov 1"),ylab="y")#,legend("topleft",paste("cor=",cor(X[,2],y))))#,#pch=vpch,
        text(x = median(as.vector(X[,2])), y = median(as.vector(y)), label = paste("cor=",round(cor(X[,2],y),4)), cex = 1)
      }
      if (k==2){
      plot(as.vector(X[,2]),as.vector(y),main=" ",xlab=paste("Cov 1"),ylab="y")#,legend("topleft",paste("cor=",cor(X[,2],y))))
        text(x = median(as.vector(X[,2])), y = median(as.vector(y)), label = paste("cor=",round(cor(X[,2],y),4)), cex = 1)
        
        plot(as.vector(X[,3]),as.vector(y),main=" ",xlab=paste("Cov 2"),ylab="y")
      text(x = median(as.vector(X[,3])), y = median(as.vector(y)), label = paste("cor=",round(cor(X[,3],y),4)), cex = 1)
      }
      if (k==3){
        plot(as.vector(X[,2]),as.vector(y),main=" ",xlab=paste("Cov 1"),ylab="y")#,legend("topleft",paste("cor=",cor(X[,2],y)),pt.bg="white", lty=c(1,2), bty="n"))
        text(x = median(as.vector(X[,2])), y = median(as.vector(y)), label = paste("cor=",round(cor(X[,2],y),4)), cex = 1)
        
        plot(as.vector(X[,3]),as.vector(y),main=" ",xlab=paste("Cov 2"),ylab="y")
        text(x = median(as.vector(X[,3])), y = median(as.vector(y)), label = paste("cor=",round(cor(X[,3],y),4)), cex = 1)
        
        plot(as.vector(X[,4]),as.vector(y),main=" ",xlab=paste("Cov 3"),ylab="y")
        text(x = median(as.vector(X[,4])), y = median(as.vector(y)), label = paste("cor=",round(cor(X[,4],y),4)), cex = 1)
      }
    }
    dev.off()
    if(k>1){
    pdf("cor_cov_cov.pdf",width=5, height=4)
    {
      if (k==2){
        par(mfrow=c(1,1))
        par(mar=c(2.8, 2.7, 1, 1))
        ...
        plot(as.vector(X[,3]),as.vector(X[,2]),main=" ",xlab=paste("Cov 2"),ylab="Cov 1")
      ...
      plot(as.vector(X[,4]),as.vector(X[,2]),main=" ",xlab=paste("Cov 3"),ylab="Cov 1")
      text(x = median(as.vector(X[,4])), y = median(as.vector(X[,2])), label = paste("cor=",round(cor(X[,4],X[,2]),4)), cex = 1)
      
      plot(as.vector(X[,4]),as.vector(X[,3]),main=" ",xlab=paste("Cov 3"),ylab="Cov 2")
    ...
    pdf("cor_matrix_cov.pdf",width=5, height=4)
    {
      par(mfrow=c(1,1))
      par(mar=c(2.8, 2.7, 1, 1))
      par(mgp=c(1.7, 0.45, 0))
