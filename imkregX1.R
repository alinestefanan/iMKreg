...
imkreg1 <- function(y,exvar.beta=NA,exvar.nu=NA,exvar.rho=NA,tau=0.5,print=T,check=F,link="logit")
{
  n <- length(y) 
  ...
  obs.inf<-function(y,muhat)
  {
    ...
    l1T <- diag(as.vector(mu.eta(A*rho)))
    num.l1T2<-dif2link(lambda1hat)
    l1T2<-diag(as.vector(-num.l1T2*(mu.eta(A*rho)^3)))
