...
imkreg0 <- function(y,exvar.beta=NA,exvar.nu=NA,exvar.rho=NA,tau=0.5,print=T,check=F,link="logit")
{
  n <- length(y) 
  ...
  dw<-function(res){
    ...
    if (n<100) {
      ...
      if (n < max(5, k)) {
        ...
        #exvar.beta<-data.frame(exvar.beta)
        #library(lmtest)
        AX <- matrix(as.vector(stats::filter(exvar.beta, c(-1, 2, -1))), ncol = k)
