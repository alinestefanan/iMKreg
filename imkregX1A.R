...
imkreg1A <- function(y,exvar.beta=NA,exvar.nu=NA,exvar.rho=NA,tau=0.5,print=T,check=F,link="logit")
{
  n <- length(y) 
  ...
  for(i in 1:n)
  {
    ...
    if(y[i]!=1) ui[i] <- pmk(y[i],alpha=z$alpha,beta=log(1-tau)/den.cr[i], lambda1=lambda1hat[i],log.p = FALSE)
  }
